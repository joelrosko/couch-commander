...
def format_light_data(light_data:dict):
    updated_response = {
        "name": light_data["name"],
        "status": light_data["state"]["on"],
        "manufacturer": "IKEA" if light_data["manufacturername"].startswith("IKEA") else light_data["manufacturername"],
    ...
    if updated_response["multicolor"]:
        updated_response["color"] = round((light_data["state"]["hue"] / 65535) * 360)
        updated_response["sat"] = light_data["state"]["sat"]
